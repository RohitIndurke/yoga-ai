...
const UpperBody = () => {
  const exercises = [
    ...
    },
    {
      ...
      icon: <Dumbbell className="h-6 w-6 text-white" style={{ transform: 'scaleX(-1)' }} />,
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
          {exercises.map((exercise) => (
            <Link key={exercise.href} href={exercise.href} className="group">
              <div className="h-full rounded-2xl border border-white/10 bg-neutral-900/50 p-6 backdrop-blur-sm transition-all duration-300 hover:border-white/30 hover:bg-neutral-800/60 hover:shadow-lg hover:shadow-white/10">
                <div className="flex items-start justify-between mb-4">
                  <div className="p-3 rounded-xl bg-white/5 ring-1 ring-white/10 group-hover:bg-white/20 group-hover:ring-white/30 transition-colors">
                    {exercise.icon}
                  </div>
                  <div className="px-3 py-1 rounded-full bg-white/5 border border-white/5 text-xs font-medium text-gray-400 group-hover:text-white transition-colors">
                    🔥
                  </div>
                </div>
                ...
                <div className="flex items-center text-sm font-medium text-gray-400 group-hover:text-gray-300">
